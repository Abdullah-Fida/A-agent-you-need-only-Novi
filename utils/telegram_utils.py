...
# ── Device identity ───────────────────────────────────────────────────
#
# Telegram binds an auth key to the device that created it. If the session is
# generated by one client identity and then used by another, that reads as a
# hijacked key and Telegram revokes it — which is exactly what "it connects
# once and then never reconnects" looks like from the outside.
#
# The login tool and the running bot therefore MUST advertise identical
# device parameters, so both take them from here and nowhere else.
...
def device_profile(seed_source: str) -> Dict[str, str]:
    """
    The device this account always presents itself as.

    Derived from the account identity rather than chosen at random, so it
    survives restarts and redeploys: a fingerprint that changed on every boot
    would look like the account roaming between handsets.
    """
    seed_source = seed_source or "novi-stealth"
    seed = int(hashlib.sha256(seed_source.encode("utf-8", "ignore")).hexdigest()[:8], 16)
    return DEVICE_PROFILES[seed % len(DEVICE_PROFILES)]


def device_kwargs(seed_source: str) -> Dict[str, str]:
    """`device_profile` as the keyword arguments TelegramClient expects."""
    d = device_profile(seed_source)
    return {
        "device_model": d["model"],
        "system_version": d["system"],
        "app_version": d["app"],
        "lang_code": d["lang"],
        "system_lang_code": d["lang"],
    }
