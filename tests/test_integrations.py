...
class TestBufferBroadcaster(unittest.TestCase):
    ...
    def _bb(self, **kw):
        return BufferBroadcaster(
            ...
            site_url=kw.get("site", "https://example.com"),
            db=kw.get("db"), ai_engine=kw.get("ai"))
    ...
    def test_article_link_uses_root_path(self):
        # The site serves articles at /{slug}, not /article/{slug}
        self.assertEqual(self._bb()._article_link("my-slug"),
                         "https://example.com/my-slug")

    def test_article_link_empty_without_site_url(self):
        self.assertEqual(self._bb(site="")._article_link("s"), "")
    ...
    def test_facebook_post_includes_required_type_metadata(self):
        ...
        bb._gql = fake_gql
        ok = asyncio.run(bb._post_to_channel(
    ...
    def test_caption_falls_back_when_ai_fails(self):
        ai = MagicMock()
        ai.generate = AsyncMock(side_effect=Exception("model down"))
        bb = self._bb(ai=ai)
        text = asyncio.run(bb._build_caption({"telegram_text": "Original body"}, "slug"))
        self.assertIn("Original body", text)
        self.assertIn("https://example.com/slug", text)

    def test_caption_empty_package_returns_empty(self):
        self.assertEqual(asyncio.run(self._bb()._build_caption({}, "")), "")
...
class TestFanout(unittest.TestCase):
    ...
    def test_one_platform_failing_does_not_stop_others(self):
        ...
        buffer = MagicMock(); buffer.post = AsyncMock(return_value=True)

        fo = Fanout(brain=make_brain(), reddit=reddit, twitter=twitter, buffer=buffer)
    ...
    def test_article_written_before_social_so_link_exists(self):
        brain = make_brain()
        brain.website_module_active = True   # website module is OFF by default
        agent = MagicMock()
        agent.generate_and_publish_article = AsyncMock(return_value={"slug": "my-slug"})
        buffer = MagicMock(); buffer.post = AsyncMock(return_value=True)

        fo = Fanout(brain=brain, article_agent=agent, buffer=buffer)
        pkg = {"telegram_text": "hi"}
        asyncio.run(fo.distribute(pkg, story={"title": "T"}))

        self.assertEqual(pkg["article_slug"], "my-slug")
        self.assertEqual(buffer.post.await_args.kwargs["article_slug"], "my-slug")

    def test_article_failure_does_not_block_social(self):
        brain = make_brain()
        brain.website_module_active = True
        agent = MagicMock()
        agent.generate_and_publish_article = AsyncMock(side_effect=Exception("ai down"))
        buffer = MagicMock(); buffer.post = AsyncMock(return_value=True)

        fo = Fanout(brain=brain, article_agent=agent, buffer=buffer)
        results = asyncio.run(fo.distribute({"telegram_text": "hi"}, story={"title": "T"}))
...
class TestWebsiteToggle(unittest.TestCase):
    ...
    def test_article_agent_runs_when_on(self):
        ...
        self.assertTrue(results["website"])
    ...
    def test_social_still_posts_while_website_off(self):
        """Turning the website off must not stop Facebook/X/Reddit."""
        brain = make_brain()
        agent = MagicMock()
        agent.generate_and_publish_article = AsyncMock(return_value={"slug": "s"})
        buffer = MagicMock(); buffer.post = AsyncMock(return_value=True)
